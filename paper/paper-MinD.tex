...
\fixme{
  ...
  \item Points out problems with Huang 2003:
    \begin{enumerate}
    \item Lacks diffusion on the membrane. Supported by Meacci 2006,
      which experimentally tries to measure diffusion on membrane, has
      a tough time, but shows that definitely diffuses.
...
of our flattened cells ($0.40\micron$).  We first solve the model's
five differential equations for the steady state solution, using for
our concentrations of MinD and MinE the same as the total in
simulation.  We then add small, spatially harmonic density
perturbations to the density, throwing away any terms that are
nonlinear in perturbation.  The sign of the eigenvalues of the
resulting matrix informs us whether harmonic perturbations at a
specified wavelength will evolve toward the steady state solution or
not, so picking a wavelength that ensures all eigenvalues are of the
correct sign allows for an upper stability limit. Through this method
we arrive at an upper half wavelength stability limit of $1.56\micron$
for our $0.40\micron$ flattened cells. As expected, when decreasing
the lengths and widths of our simulated flattened cells so that the
