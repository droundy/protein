...
\fixme{
  First referee:
  \begin{enumerate}
  \item \blue{David} Add discussion of ``platform'' --- wrote our own code,
    publically available (cite github?).
  \item \blue{David} Make 3D geometries available? Cite that the publically
  ...
  \item Points out problems with Huang 2003:
    \begin{enumerate}
    \item \blue{David.  I've added a paragraph discussing the starry
      night in the context of the membrane diffusion.  I didn't say
      anything about a one half period allowing diffusion to smear an
      entire pole, because it seems to contradict the next paragraph.
      What do you think?} Lacks diffusion on the membrane. Supported
      by Meacci 2006, which experimentally tries to measure diffusion
      on membrane, has a tough time, but shows that definitely
      diffuses.
    ...
    \end{enumerate}
  \item \blue{David} - Height should be bigger to compare
    with experiment.  0.4-0.6 microns.  \textcolor{blue}{HIGH
...
silicon channels of thickness less than that of the natural diameter
of the cells. Upon entering the channels, the cells undergo a
mechanical deformation in which they both widen and lengthen within
the plane of the channel.  This deformation results in very wide (they
can reach widths of over $5\micron$) flattened cells that when viewed
from the top down have irregular and asymmetric shapes.  While these
cells are still able to divide into surprisingly equal volumes, the
MinD oscillations in these cells are irregular, both temporally and
spatially. Seen with fluorescent microscopy, the MinD maximize in
multiple locations within the cell, in a seemingly random
sequence. These experiments allow for an opportunity to test MinD
simulation models against more extreme cases than have been seen thus
far.
...
(following experimentally observed non-linear attachment of MinD on the cell
...
$\delta(d_w)$, which we use to describe the location of the membrane,
...
deviate from the perameter values used in the cited work.
...
respectively.  We have chosen to write our own simulation
platform and membrane creation tools from scratch, instead of using
existing available software used in previous studies\fixme{cite
  previous studies} (we believe there is value in re-creating results
with alternative software).  Our simulations take place within a
three-dimensional Cartesian grid that has a grid spacing of
.05\micron, and our cell shapes are specified as the zero of an
analytic function which can generate the geometry presented in this
paper.  Our code is publically available on github.com~\fixme{cite
  github account in references?}
...
Mannik \emph{et al.}, in which bacteria are confined within thin
slits.  \blue{(6)}These slits are fabricated with a width of
.25\micron, but they are coated with a PDMS lining that the cells are
able to deform, raising uncertainty in the actual cell thickness.
Tests with pure silicon, non-deformable slits show that the greatest
thickness for which cells are able to penetrate is
...
\begin{figure}
  ...
  \caption{Temporal correlation function of the total MinD found in
    ...
    oscillation period of 39.5 seconds and coherence time of 307
    seconds.  The correlation functions are scaled to have the same
    initial value.}
  \label{corr-pill}
\end{figure}
...
forming on the wall and then subsequently dissipating.  We attribute
this effect to the fact that the Huang model does not include
diffusion along the membrane, which has been experimentally verified
yet found to be two orders of magnitude less than diffusion in the
cytoplasm~\cite{meacci2005min}. Even ignoring this effect, however, it
is clear from Fig.~\ref{image-p} that the stochastic simulation
results in a protein distribution that is less spatially regular than
the deterministic model predicts.  This naturally leads to
...
\begin{figure}
  ...
  \caption{Temporal correlation function of the total MinD found in
    ...
    coherence time of 246 seconds, so that it takes roughly 4.6
    periods for the behavior to decohere.  For the stadium \emph{A},
    the same model shows an oscillation period of \fixme{following not
      correct anymore} 51 seconds and coherence time of 323 seconds,
    so that it takes roughly 6.3 periods for the behavior to
    decohere.}
  \label{fig:corr-pancake-A}
\end{figure}

\section{Comparison of Experimental and Stadium Shapes}
...
deterministic model shows uniformly bipolar oscillation.  From these
results, we conclude that the deterministic model is inadequate to
explain experimental observations of the locations of density maxima
of the MinD protein.
...
viewed from above that primarily causes the disruption of regular
bipolar oscillation which is observed by Mannik \emph{et
