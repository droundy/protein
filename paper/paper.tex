...
seek out the extreme poles in the cell~\cite{corbin2002exploring,
  juarez2010changes}.  One conclusion of these studies has been that
the MinD system robustly forms bipolar oscillations regardless of
variations in cellular shape.
...
that when viewed from the top down show irregular and asymmetric
shapes.  While these cells are still able to divide into surprisingly
equal volumes, the MinD oscillations in these cells is irregular,
...
recruited to the membrane by MinD is already clustered there
...
made~\fixme{finding what results? We don't want to just list what has
  been done, but rather to summarize the state of
  knowledge.}~\cite{hsieh2010direct, mileykovskaya2003effects}.
...
approximation developed by Zhang \emph{et al.}~\cite{zhang2007gaussian}.  In
...
shows one period of oscillation.  At $t=0$ the cytoplasmic MinE is
...
time before spontenously undergoing nucleotide exchange and shifting
...
the protein into the MinE \fixme{collection ??}, where reactions will prevent it
...
We present simulations of a cell with radius $.5\micron$ and length
$4\micron$, measured from the tip of one end cap to the tip of the
other. We observe in the deterministic model a quick establishement
(within a few periods) of regular oscillatory behavoir that lasts
indefinitely.  This temporal regularity can be seen in
...
In the case of the stochastic model the pill exhibits coherence times
of over 7 periods and in the case of the determinisitic model
oscillate indefinately with complete coherence.
...
model maxima location.  \fixme{I think we should discuss stochastic
  picture earlier.}
