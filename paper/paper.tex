...
selection and oscillation in a wide variety of cell shapes.  However
we have found that this robustness is heavily dependent upon cell
size.  Two pole, regular oscillatory behavoir is found in cells which
are long enough in one dimension for the reaction chain to be unstable
but short enough in the perpendicular dimension to be stable.  The
limits are seen when both dimensions are too short to be unstable, so
that initial assymetry in protein concentration relaxes into a
motionless steady state, or when both are too great, so that the
proteins exhibit much more complex and varied behavoir.
...
shows a characteristic half wavelength in perturbative spatial
...
oscillations seem to break down when the shortest distance across the
cell is approximately equal to the characteristic half wavelength at
the particular thickness at which we simulate (our simulations of
cells with thickness $0.25\mu m$ corresponds to half wavelengths of
$2.13\mu m$).  Huang~\cite{huang2003dynamic} performs a similar
analysis for a cylindrical shape \fixme{is it cylindrical or 1d?} and
finds characteristic half wavelengths of $2\mu m$.
...
steadily between them.  However, when we keep the same shape but
increase the size, we see that there is a critical size at which the
protein dynamics lose their order and become very complex (showing
hints of prefered wall maxima locations), similar to what Mannik sees
experimentally.  We find that this critical size matches up well with
our stability analysis, in that the critical sizes correspond to sizes
for which the shorter of the two distances across the cell is roughly
equal to the analysis' half wavelength found for this cell thickness.

\begin{figure}
  \includegraphics[width=3cm]{../data/shape-randst/plots/box-plot_D--randst-25-800-800-9500-1500}
  \includegraphics[width=3cm]{../data/shape-randst/plots/box-plot_D--randst-25-1000-1700-9400-1500}
  \caption{Box plots of Mannik shapes for sizes at the lower stability
    threshhold.  Both shapes have longer axis of roughly 2.2$\mu m$.}
  \label{box-mannik}
\end{figure}
Decreasing the cell size in succesive steps, we can see as well the
critical size at which the cell oscillations are quickly damped out.
Figure~\ref{box-mannik} shows box plots of the largest cells simulated
whose oscilltions relax into the steady state solution.  Both have a
long axis of roughly 2.2$\mu m$, in agreement with theory.


\fixme{I'm also waiting for data to come in that will hopefully show
  cells so small that no oscillations take place.  We can put this in
  as well}

\section{Triangular Shapes}

Still waiting for the triangle data to show what we expect.  We have
small triangles that gradually go from no oscillations to
oscillations, but the large triangle data does not yet show
irregularities.  I have very large simulations running that should
definitely show this at some point (Give it a few days).
