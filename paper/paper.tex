...
MinE, in such a fashion as to exhibit pole to pole oscillatory
...
and diffusion~\cite{fange2006noise, kerr2006division}\fixme{talk about
  this awkward sentence}.  These studies largely confirm the results
of Huang's model when applied to the wild-type, pill-shaped phenotype.
However, the stochastic models more successfully predict
...
state solution of a $2\micron$ half wavelength.  Cells with dimensions
longer than this show spontaneous oscillatory behavoir in this
...
simulated in excess of 15000 seconds (over four hours) of evolution of
the MinD system using the deterministic version of Huang's
model~\cite{huang2003dynamic}, and much more than this using the
stochastic model. \fixme{is this sentence ok?}
...
\begin{figure}
  ...
  \caption{Temporal correlation function of the total MinD found in
    ...
    stochastic models are shown.  For shape \emph{A}, the stochastic
    model shows an oscillation period of 53.1 seconds and decay time
    of 246 seconds, so that it takes roughly 4.6 periods for the
    behavoir to decohere.  For the stadium \emph{A}, the same model
    shows an oscillation period of 51 seconds and decay time of 323
    seconds, so that it takes roughly 6.3 periods for the behavoir to
    decohere.}
...
cell shape irregularity versus overall size and aspect ratio in
developing an understanding of the origin of the experimentally
observed behavior.  Although in the movies the stadiums appear to
display, on average over long time scales, a more regular bipolar
oscillation than the irregular mannik shapes, the limited time range
of the arrow plots (350 seconds) is inadequate when studying this
difference in behavoir.  We therefore return to the correlation
...
cell.  For each shape we computed these correlation functions using
the entirety of our over 15000 seconds of simulation
data. Fig.~\ref{fig:corr-pancake-A} shows this plot for shape \emph{A}
and stadium \emph{A}.  E. Coli cell doubling times vary widely
depending on environmental conditions, but will often be between 20
minutes to 100 minutes~\cite{pierucci1972chromosome}, while
...
correlation times up to 600 seconds, a biologically relevent
timescale.
...
factor, so that they can be directly compared against eachother.  At
$\tau=0$, the stochastic correlation is already seen to be lower than
the deterministic, reflecting the fact that the stochastic data
sometimes shows maxima or minima at both ends simultaneously.  The
...
coherence time of 6.3 periods.  This moderate difference in coherence
time shows that the regularity in cell oscillation is not overly
sensitive to the regularity and symmetry of the shape.  It seems that
it is the size and aspect ratio that are more important in dictating
the regularity of bipolar oscillation.
...
deterministic version of Huang's widely used reaction-diffusion
model. We find that the deterministic model~\cite{huang2003dynamic}
predicts behavior that contradicts what is seen experimentally, namely
robust and periodic bipolar oscillation in irregularly shaped cells,
with a minor intermediate maximum appearing in shape \emph{B}.  Thus
we conclude that this method is inconsistent with experiment, and
should not be relied upon for predictions of the behavior of the MinD
system.
...
predicts qualitative behavior in wild-type, pill-shaped cells that is
broadly consistent both with experiment and with the deterministic
model. However, even in these simple pill-shaped cells the stochastic
model shows variation in the location of MinD maxima on either end of
the cell. This model also predicts spatially irregular formation of
maxima for all the flattened cells that we studied, in agreement with
experiment.  In order to better understand the irregular oscillations
...
of irregular shape.  We did not find a significant difference in the
coherence of bipolar oscillations within these stadium shapes and the
irregular shapes observed by Mannik \emph{et al.}.  Thus we conclude
that the disruption of the MinD oscillations seen by Mannik \emph{et
  al.}  is largely due to the flattened cell shape and greater than
average size, as opposed to its irregularity.
