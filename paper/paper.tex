...
%
The correlation function for stadium \emph{B} (5.5 periods, not shown)
is similar to that of stadium~\emph{A}, with similar coherence time,
while shape~\emph{B} exhibits a lower coherence time (2.2 periods).
In the deterministic model, shape~\emph{B} is not precisely periodic
on biologically relevant timescales, which is reflected in
...
oscillatory behavior, leaving the polar oscillation largely intact,
with even the very distorted shape~\emph{B} exhibiting only a factor
of two greater decoherence.

\section{Conclusion}

\fixme{Stochastic model successfully explains experiment.}

\fixme{Deterministic is terrible.}
%
...
\fixme{What is the point here? Should it be cut?}
%
...
\fixme{Observations of irregularity would be seen even in regularly
  shaped flattened or pancake cells.}
%
...
the effect of irregular shape.  We did not find a significant
difference in the coherence of bipolar oscillations within these
stadium shapes and the irregular shapes observed by Mannik \emph{et
  al.}.  Thus we conclude that the disruption of the MinD oscillations
seen by Mannik \emph{et al.}  is largely due to the flattened cell
shape and greater than average size, as opposed to its irregularity.

\fixme{Talk about flattened cells throughout vs. irregular cells.}
