...
recruitment to the membrane, prohibiting MinD from attaching to
MinD-MinE complexes there, behavior that is experimentally unresolved.
The model of Bonny \emph{et al.} allows MinE to remain independently
...
stochastically simulate the similar reaction-diffusion
equations~\cite{fange2006noise, kerr2006division}.  These studies
largely confirm the results of Huang's deterministic model when
applied to the wild-type, pill-shaped phenotype.  However, the
stochastic models are slightly more successful in predicting
...
model, which would allow these recruitment clusters to spread out.
...
irregular cells are large enough so that the one micron blur would not
completely eliminate the `starry night' variation in maxima position
effect.
