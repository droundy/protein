...
\section*{Abstract}
  ...
  \changed{predicts more spatially irregular oscillations}.
  We further report simulations of flattened but more symmetric
  shapes, \changed{which suggest that the flattening and lateral expansion may
...
\section*{Author Summary}
  When an E. \changed{coli} cell divides it must split into two relatively equal
...
states~\cite{meinhardt2001pattern}.  \changed{Kruse extended this model by
...
Several models \changed{\cite{bonny2013membrane,
...
\changed{applications} of the Huang 2003 model that stochastically simulate the
...
behavior~\cite{kruse2007experimentalist}\changed{, including the observation of
...
used~\changed{\cite{hattne2005stochastic,huang2004min,kerr2006division,
...
\changed{This allows us to test the performance of this important model in more
...
\begin{figure}
  ...
  \caption{Reactions included in the \changed{Min system} model of Huang
...
and then subsequently dissipating.  \changed{This result, which is not observed
...
one micron blurring \changed{might} not completely eliminate this `starry night'
effect.
...
\begin{figure*}
  ...
  \caption{\changed{Arrows} depicting successive maxima in space
...
that \changed{this} model is \emph{not} chaotic (which would show irregularity
...
density maxima of the MinD protein.  \changed{We note, however, that it is
...
The predictions of the stochastic model \changed{show some} similarity to the
experimental results in terms of irregularity of the locations of
maxima, \changed{but the stochastic model displays significantly less
...
that is qualitatively similar to our irregular shapes\changed{: in both
...
bottom of the cell.  \changed{The ``top'' and ``bottom'' regions for computing
...
to 500 seconds, a biologically relevant timescale.  E. coli cell
...
shape \changed{leads to a decrease} in temporal regularity of the oscillatory
behavior, although even the very distorted shape~\emph{B} exhibiting
only a factor of two greater decoherence.

\section*{Conclusion}
We find \changed{qualitative} agreement between our stochastic simulations and
experiments showing disrupted bipolar MinD oscillation in asymmetric
flattened E. coli cells~\cite{mannik2012robustness}.  As observed
experimentally, our simulations predict that MinD maxima will form in
a spatially irregular sequence of locations, \changed{which follow an overall
...
deterministic case also allow us to conclude that \changed{Huang's} model is
non-chaotic.  Because of their similar construction, we anticipate
that recent variants of the model~\cite{bonny2013membrane,
  fange2006noise, halatek2012highly} \changed{may} also prove to be
non-chaotic.
...
it \changed{may be} the flattening of the cells, and the accompanying
expansion in the transverse direction, \changed{that causes} the disruption of
