...
\fixme{This allows us to test the performance of this important model
  in more extreme situations than has been done previously, furthering
  considerably our understanding of how and to what extent it breaks
  down. It will be valuable in the future to compare these results
  against similar tests of more recent models, in order to better
  understand how they gain advantage.}
...
experiment shows maxima occurring that we never see in our
simulations, suggesting that the model does not precisely reflect the
experiment.  We also note that the stadium shapes in
Fig.~\ref{randst-plot-ave} appear to have maxima location irregularity
\fixme{within the time domain of a small number of periods, as
  compared to the more long term coherence discussed below,} that is
qualitatively similar to our irregular shapes and to experiment.
...
shape has \fixme{a distinct yet moderate} affect on the temporal regularity of
the oscillatory behavior, with even the very distorted shape~\emph{B}
...
it is \fixme{primarily} the flattening of the cells, and the
accompanying expansion in the transverse direction, \fixme{deleted
  some text} that causes the disruption of regular bipolar oscillation
which is observed by M\"annik \emph{et
